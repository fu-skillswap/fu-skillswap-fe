...
export const MenteeBookings: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-left">
      ...
      {successMsg && (
        <div className="flex items-start gap-3 bg-green-50 border border-green-200 text-green-700 p-4 rounded-xl text-xs font-semibold animate-fadeIn">
            ...
            >
              ...
              <div className="space-y-3 flex-1 text-left">
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <span className="text-xs font-bold text-brand-text block">{b.mentorName}</span>
                    <span className="text-[10px] text-brand-terracotta font-bold">{b.skill}</span>
                  </div>
                ...
                <div className="space-y-1 bg-brand-bg/40 border border-brand-border p-3.5 rounded-2xl">
                  ...
                  <p className="text-[11px] text-brand-text-muted font-medium pl-5 leading-relaxed">
                    {b.learningGoal}
                  </p>
                </div>
              ...
              <div className="flex md:flex-col justify-end items-end gap-2 shrink-0">
                {b.status === 'PENDING' && (
                  <span className="text-[11px] text-brand-text-muted font-bold bg-brand-bg border border-brand-border px-3.5 py-2 rounded-xl">
                ...
                {b.status === 'ACCEPTED' && (
                  <div className="flex flex-wrap gap-2 justify-end">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-1.5 bg-brand-blue hover:bg-brand-blue-hover text-white text-xs font-bold py-2.5 px-4 rounded-xl cursor-pointer shadow-md shadow-brand-blue/20 transition-all active:scale-95"
                    >
                      <Video className="w-3.5 h-3.5" /> Vào lớp học
                    </a>
                    <button
                      onClick={() => handleSimulateComplete(b.id)}
                      className="flex items-center gap-1 bg-white hover:bg-brand-bg border border-brand-border text-brand-text text-xs font-bold py-2.5 px-3.5 rounded-xl cursor-pointer transition-all active:scale-95"
                    >
                      Kết thúc buổi học
                    </button>
                  </div>
                  ...
                  <div className="space-y-1.5 text-right">
                    {b.isReviewed ? (
                      <div className="space-y-1 text-right">
                        <span className="text-[10px] text-green-700 font-bold bg-green-50 border border-green-200 px-3 py-1.5 rounded-xl block">
                          Đã gửi đánh giá
                        </span>
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 bg-brand-terracotta hover:bg-brand-terracotta-hover text-white text-xs font-bold py-2.5 px-4 rounded-xl cursor-pointer shadow-md shadow-brand-terracotta/20 transition-all active:scale-95 animate-bounce"
                      >
                ...
                {b.status === 'REJECTED' && (
                  <span className="text-[11px] text-red-600 font-bold bg-red-50 border border-red-100 px-3.5 py-2 rounded-xl">
                    Đã từ chối lịch hẹn
                  </span>
                )}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-lg bg-white border border-brand-border rounded-3xl p-6 relative shadow-2xl text-left space-y-4">
            ...
            <form onSubmit={handleReviewSubmit} className="space-y-5">
              ...
              {/* Star Rating */}
              <div className="text-center py-2 bg-brand-bg/40 border border-brand-border rounded-2xl space-y-1.5">
                <span className="text-[10px] font-bold text-brand-text-muted uppercase tracking-wider block">Độ hài lòng chung</span>
                ...
                <span className="text-xs font-bold text-brand-terracotta block">
              ...
              <div>
                <label className="block text-xs font-bold text-brand-text-muted mb-1.5">Góp ý riêng cho Mentor (Bảo mật, chỉ Mentor xem)</label>
                <textarea
                  ...
                  className="w-full bg-brand-bg/50 border border-brand-border rounded-xl p-3 text-xs text-brand-text focus:outline-none focus:border-brand-terracotta resize-none placeholder-brand-grey font-medium"
                />
              </div>

              {/* Public Review */}
              <div>
                <label className="block text-xs font-bold text-brand-text-muted mb-1.5 flex items-center gap-1">
              ...
              <div className="flex items-center gap-3">
                <label className="relative flex items-center cursor-pointer">
                  ...
                  <span className="ml-3 text-xs font-bold text-brand-text-muted">Đồng ý hiển thị đánh giá này công khai</span>
                </label>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowReviewModal(false)}
                  className="w-1/3 py-3 rounded-xl border border-brand-border hover:bg-brand-bg text-brand-text text-xs font-bold cursor-pointer transition-all active:scale-95 text-center"
                >
                  Hủy
                </button>
                <button
                  type="submit"
                  className="flex-1 py-3 rounded-xl bg-brand-terracotta hover:bg-brand-terracotta-hover text-white text-xs font-bold cursor-pointer transition-all active:scale-[0.98] shadow-md shadow-brand-terracotta/25 flex items-center justify-center gap-1.5"
                >
