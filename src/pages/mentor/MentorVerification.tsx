...
export const MentorVerification: React.FC = () => {
  ...
  const Banner = () => {
    const map: Record<VerificationStatus, { title: string; desc: string; cls: string; icon: React.ReactNode }> = {
      DRAFT: { title: 'Hồ sơ đang ở bản nháp', desc: 'Hoàn thiện checklist bên dưới rồi nộp để admin duyệt.', cls: 'bg-brand-bg border-brand-border text-brand-text', icon: <FileText className="w-5 h-5" /> },
      ...
      REJECTED: { title: 'Hồ sơ bị từ chối', desc: req?.reviewNote || 'Hồ sơ chưa đạt yêu cầu.', cls: 'bg-red-50 border-red-200 text-red-700', icon: <XCircle className="w-5 h-5" /> },
      WITHDRAWN: { title: 'Bạn đã rút hồ sơ', desc: 'Hồ sơ xác thực đã được rút.', cls: 'bg-brand-bg border-brand-border text-brand-text-muted', icon: <Undo2 className="w-5 h-5" /> },
    };
    const b = map[status];
    return (
